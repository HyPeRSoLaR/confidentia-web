...
import { Send, Mic, Video, MessageCircle, Loader2, Check } from 'lucide-react';
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-3">
            {!sessionEnded && (
              <Button onClick={() => setSessionEnded(true)} className="bg-coral/10 text-coral hover:bg-coral hover:text-white border-0 shadow-none transition-all duration-300 rounded-xl px-4 text-sm font-medium">
                End Session
              </Button>
            )}
            <div className="flex bg-surface rounded-xl p-1 border border-border gap-1 hidden sm:flex">
              {MODES.map(m => {
                const Icon = m.icon;
                return (
                  <button
      ...
      {sessionEnded ? (
        ...
        >
          ...
          <p className="text-muted text-base max-w-sm leading-relaxed">
            Your conversation has been securely saved and encrypted. Take a moment to breathe and reflect on your progress today.
          </p>
          <div className="pt-8">
