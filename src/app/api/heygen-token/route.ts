...
// Each avatar uses its OWN native voice with language:'fr' for French timbre
// All 20 LiveAvatar voices accept language:'fr' — confirmed via API testing
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (useLiteMode) {
      ...
      // FULL mode — each avatar uses its own native voice + language:'fr'
      requestBody = {
        mode:      'FULL',
        avatar_id: avatarId,
        avatar_persona: {
