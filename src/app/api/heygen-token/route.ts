 ...
 *
 * To support 8 distinct French voices, we use 8 distinct LiveAvatar Projects.
 * Each project has a specific default French voice set on the HeyGen Dashboard,
 * and thus a specific API Key.
 *
 * We map the incoming `avatarName` to its corresponding environment variable:
 * e.g., "Anna" -> LIVEAVATAR_API_KEY_ANNA
 *
 * If the specific key is missing, it falls back to LIVEAVATAR_API_KEY.
 */
...
export async function POST(req: Request) {
  ...
  // Find the specific API key for this avatar (e.g. LIVEAVATAR_API_KEY_ANNA)
  const envKeyName = `LIVEAVATAR_API_KEY_${avatarName.toUpperCase()}`;
  const apiKey = process.env[envKeyName] || process.env.LIVEAVATAR_API_KEY;

  if (!apiKey) {
    return NextResponse.json(
      { error: `Missing API Key for ${avatarName} (${envKeyName} or LIVEAVATAR_API_KEY)` },
      { status: 500 },
    );
  }

  console.log(`[LiveAvatar] Mode: FULL | Avatar: ${avatarName} (${avatarId}) | Key: ${envKeyName} (Present: ${!!process.env[envKeyName]})`);
  ...
  try {
    const requestBody = {
      ...
      avatar_id: avatarId,
      avatar_persona: {
        // We DO NOT pass voice_id anymore. The default voice is configured 
        // on the HeyGen dashboard for each respective API Key.
        language:   'fr',
